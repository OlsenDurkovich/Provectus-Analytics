...
- **Exec one-pager / printable summary — NEW (Olsen, 2026-06-30).** A single shareable summary for the boss / leadership: headline KPIs (ratings completed, active students, total billed, flight hours), per-rating cost+hours+days table, and 2–3 insight highlights (most-efficient instructor, the cadence-vs-time finding, any at-risk count). Export as PDF (a `tools/build_boss_pdf.py` already exists as a starting point) or a print-styled HTML view. Bosses want something to take away; this is the artifact. Low complexity, high demo value.
