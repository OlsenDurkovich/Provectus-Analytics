...
- **Stage-check pipeline ingest** (added 2026-06-28, Olsen) — wire the new **Provectus Stage Check Record** Google Form into attribution. Each stage check becomes a confirmed, dated, rating-labeled pointer (a *hard* window boundary, outranking the guesstimate heuristic and even the retroactive survey for future students). New `stage_checks` table + `ingest_stage_checks()`, reuse `reconcile.py` to join on email→name, and have `build_enrollments`/`milestones` prefer stage-check dates (pre-solo→`first_solo`, IFR Stage 1→`xc_pic_complete`, pre-checkride→rating-end window). Needs an unmatched-rows surface (instructors may not know the exact FSP email). **Full design + schema + build steps: `docs/2026-06-28-stage-check-integration-spec.md`.** Build once the form's response sheet is linked with a few real rows. Form + form spec already done (`Stage Check Form Builder.gs`, `Stage Check Form Spec.md`).
