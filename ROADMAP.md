...
- **Insights / Analytics tab — NEW (Olsen, 2026-06-30).** A dedicated tab for *derived* analytics on top of the raw dashboards. Five asks:
  1. **At-risk alert** — flag a student **X% over the cohort median** (hours/cost/days) for a rating → "may need more help." Threshold X TBD (~25–30% start). Data ready: per-rating norms (median + P25/P75) already computed in `norms.compute_rating_norms`.
  2. **Instructor ↔ rating strengths** — which instructor is best at which rating (lowest hours/cost-to-checkride for their students, per rating). Builds on the existing per-instructor/per-rating aggregates.
  3. **Instructor efficiency ranking** — who's more efficient overall vs cohort median.
  4. **Completion prediction** — predict when a student will finish a rating from their pace/progress → drives **when to schedule the checkride**. Harder/speculative; needs a pace model.
  5. **Cadence vs cost** — does flying **3×/week** cost less than 2×/week because the student finishes quicker and retains more (fewer re-teach hours)? Correlate events/week (from flight dates) against total rating cost + duration. Most novel insight; also the most data-hungry.
  Honest caveat: with ~20 alumni + synthetic data, #4 and #5 have thin statistical power until real survey + FSP data lands — build the framing now, trust the numbers later. Memory: [[project-insights-tab]].
