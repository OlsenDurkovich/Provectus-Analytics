...
- **Insights / Analytics tab — ALL 5 BUILT (Olsen, 2026-06-30).** A dedicated tab for *derived* analytics on top of the raw dashboards.
  1. **At-risk alert — DONE** (PR #26). Flags students ≥ threshold (25%/10%/50% toggle) over the cohort median on hours or cost.
  2. **Instructor ↔ rating strengths — DONE** (PR #26, refined #27). Per rating, instructors ranked by avg hours-to-checkride vs the *other* instructors' students (leave-one-out).
  3. **Instructor efficiency ranking — DONE** (PR #26/#27). Overall leave-one-out deviation, ranked.
  4. **Completion prediction — DONE** (2026-06-30, `feat/insights-predict-cadence`). Projects in-progress students' checkride-readiness from recent flight pace vs cohort median; flags on-track / over-median / stalled. Required surfacing in-progress students first (guesstimate fallback — see below).
  5. **Cadence vs cost — DONE** (same branch). Completed PPL students bucketed by flights/week → avg hours, cost, calendar days. Synthetic "cadence cohort" (12 PPL completers, `generate_synthetic_fsp_data.py::CADENCE_COMPLETERS`) bakes in a retention gradient so the relationship is visible; CLEARLY SYNTHETIC/illustrative until real data.
  **Pipeline change that unblocked #4:** in-progress students (no checkride) used to get NO enrollment (guesstimate required `billing == PRIMARY`; synthetic billing is NULL). Added a fallback — single-engine Dual Flight Training with no billing → PARTIAL PPL enrollment — so active students surface (as Active/On-checkride clients) without entering cohort norms (no checkride milestone). Also made synthetic invoices deterministic per-reservation so adding students can't perturb existing alumni costs. Honest caveat: with ~20 alumni + synthetic data, #4/#5 numbers are illustrative until real survey + FSP data lands. Memory: [[project-insights-tab]].
- **Exec one-pager / printable summary — DONE (2026-06-30, `feat/exec-one-pager`).** New `/summary` route (`routes/Summary.tsx`), overview-gated, in nav + Cmd-K. One screen: headline KPIs (all-time), per-rating cost/hours/days table (cohort medians), and three highlight cards (most-efficient instructor, the cadence-vs-time finding, in-progress + at-risk counts). "Print / Save PDF" button → `window.print()` with an `@media print` stylesheet that hides the sidebar/topbar and prints clean on one page. Reuses existing endpoints (kpis, ratings-bars ×3 metrics, insights, meta) — no new backend. Synthetic-data banner shown until `meta.mode === 'real'`.
...
- **Guard the "Rebuild DB" button — DONE** (2026-06-28) — `RebuildConfirmDialog` now gates every rebuild trigger (sidebar + Cmd-K): admin must type "REBUILD" to arm it, with a clear warning + a "Rebuilding…" running state. No more accidental one-click resets. (Accounts are also safe regardless now — separate auth DB.) *Remaining stretch (optional):* move the rebuild to a background task so a long run can't tie up a request — lower priority now that the swap is atomic/crash-safe.
- **Compartmentalize `flight_overrides` like users** (added 2026-06-28, Olsen) — follow-on to the auth-DB split. Manual per-flight corrections are durable user input but still live in the analytics DB. Real-data rebuilds re-apply them (`ingest.apply_overrides`), but a *synthetic* rebuild wipes them. Not urgent (prod is synthetic; they self-restore once real exports land), but it's the same durable-vs-regenerable mix. Options: move overrides to the dedicated auth/system DB (FK to `flights.flight_id` can't cross DBs, so key by `fsp_reservation` instead and re-apply after every rebuild), or snapshot/re-apply them in the synthetic path too. Best-practice goal: durable user input never lives in a file the rebuild replaces. (Users + settings already done — auth DB, `feat/separate-auth-db`.)
