...
- **User & access management — DONE, MERGED + DEPLOYED** (2026-06-27, PRs #9–#13). Roles admin/instructor/viewer, admin user-management UI + API, self-service password change, last-admin guard — PLUS **per-user page access** (admins tick exactly which pages each user sees; enforced server-side via `require_page()`, not just UI) and **admin password reset**. Verified live on provectusanalytics.com. Specs: `docs/2026-06-27-user-access-management-spec.md`, `docs/2026-06-27-per-user-access-spec.md`.
  - *Remaining follow-up — instructor data scoping:* make the `instructor` role see only their own students (link a user account to an FSP instructor identity, then filter the `students`/`instructors` queries). Not built. The **student role (PR #14) now establishes the pattern**: link a user to an identity, then serve a dedicated endpoint scoped to the token-derived id — the instructor version would do the same but for a filtered students/instructors view rather than a single record. NOTE the honest caveat: plain page access controls which pages/endpoints a user reaches, not a per-field data wall; real isolation needs a scoped endpoint like `/api/me/training`.
