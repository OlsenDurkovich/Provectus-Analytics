...
def test_pipeline_matches_ground_truth(pipeline_db):
    ...
    # The ground truth is the answer key for the SURVEY alumni. Non-survey
    # students (in-progress + the cadence-demo cohort) legitimately add milestones
    # via guesstimate, so only hold the pipeline to exactness for alumni that are
    # actually in the answer key.
    gt_clients = {r["client_fsp"] for r in gt}
    missing = sorted(set(gt_index) - set(out_index))
    extra = sorted(k for k in set(out_index) - set(gt_index) if k[0] in gt_clients)
    assert not missing, f"missing from pipeline: {missing[:10]}"
    assert not extra, f"extra for a ground-truth alum: {extra[:10]}"
...
def test_pipeline_total_billed_matches(pipeline_db):
    ...
    # Spot-check a few against the ground-truth answer key (invoices are now
    # seeded per-reservation, so these are stable regardless of other students).
    assert abs(by_alum["Alex Martinez"] - 65992.00) < 0.01
    assert abs(by_alum["Sofia Garcia"] - 14878.50) < 0.01
    assert abs(by_alum["Brandon Lee"] - 20342.50) < 0.01
